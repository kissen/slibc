...
ZFILE *FIGopen(name, suffix) char *name;
char *suffix;
{
	char *fontpath;
	ZFILE *fontfile;
	...
	fontpath = (char *)alloca(sizeof(char) * (namelen + MYSTRLEN(name) + MYSTRLEN(suffix) + 2));
	fontfile = NULL;
	if (!hasdirsep(name))
...
void readfont()
{
	...
	numsread = sscanf(fileline, "%*c%c %d %*d %d %d %d %d %d", &hardblank, &charheight, &maxlen,
					  &smush, &cmtlines, &ffright2left, &smush2);
...
inchr iso2022()
{
	...
	default:
		if (ch & 0x200)
		{ /* set G0 to 94 x 94 char set (deprecated) */
			gn[0] = (ch & ~0x200) << 16;
			gndbl[0] = 1;
			return iso2022();
		}
