...
#define RUNTIMECHECK                                                                               \
	if (!ZS || (ZS->runtimetypeid1 != ZIPIOSTATETYPE) || (ZS->runtimetypeid2 != ZIPIOSTATETYPE))   \
		return -1;
#else
#define RUNTIMEINIT
#define RUNTIMECHECK
...
static int BufferRead(struct ZipioState *zs, long offset, unsigned char *buffer, long length)
{
	...
	if (BufferPump(zs, offset + length))
		return TRUE;

	/* If using file buffering, just get the data from the file */
	if (zs->tmpfil)
	{
		if (fseek(zs->tmpfil, offset, SEEK_SET))
			return TRUE;
		if (fread(buffer, 1, (size_t)length, zs->tmpfil) != length)
			return TRUE;
	}
...
static int BufferAppend(struct ZipioState *zs, unsigned char *buffer, long length)
{
	...
	if (zs->tmpfil)
	{
		if (fseek(zs->tmpfil, zs->outinf, SEEK_SET))
			return TRUE;
		if (fwrite(buffer, 1, (size_t)length, zs->tmpfil) != length)
			return TRUE;
	}
	/* If no temp file, use memory buffering */
	else
...
)
{
	...
	/* Append to the buffer */
	if (BufferAppend(ZS, buffer, length))
		return TRUE;
...
ZFILE *Zopen(const char *path, const char *mode)
{
	 ...
	 */
	if ((zs->sign != ZIPSIGNATURE) || (zs->flag & 1) || (zs->comp != 8) ||
	...
	else
	{
		...
		zs->inflatestate =
			InflateInitialize((void *)zs, inflate_putbuffer, inflate_malloc, inflate_free);

		if (InflatePutBuffer(zs->inflatestate, zs->inpbuf + 30 + zs->flen + zs->elen,
							 inplen - 30 - zs->flen - zs->elen))
...
int _Zgetc(ZFILE *stream)
{
	...
	if (ZS->getoff != offset)
	{
		if (BufferRead(ZS, offset, ZS->getbuf, length))
			return -1;
...
int Zseek(ZFILE *stream, long offset, int whence)
{
	...
	if ((newoffset < 0) || (newoffset > ZS->usiz))
		return -1;
