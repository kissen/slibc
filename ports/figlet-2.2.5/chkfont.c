...
void weregone(really) int really;
{
...
int badsuffix(path, suffix) char *path;
char *suffix;
{
	char ucsuffix[10];
	char *s;
...
void checkit()
{
	...
	numsread = sscanf(fileline, "%c %d %d %d %d %d %d %d %d", &hardblank, &charheight, &upheight,
					  &maxlen, &old_layout, &cmtcount, &ffrighttoleft, &layout, &spectagcnt);
	free(fileline);
	fileline = NULL;
	if (numsread < 7)
	{
	...
	if (have_layout && old_layout == 0 && (layout & 192) != 64 && (layout & 255) != 128)
	{
	...
	if (have_layout && old_layout > 0 && (!(layout & 128) || old_layout != (layout & 63)))
	{
